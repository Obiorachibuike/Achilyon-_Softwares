...
const App = () => {
  ...
  const fetchData = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      // If it's initial or dashboard view, we might want to keep a separate set for global trending
      if (isInitial || view === 'dashboard') {
        const trending = await coinService.getTrending()
        setDashboardCoins(trending)
      }
  ...
  useEffect(() => {
    fetchData(true)
  }, [fetchData]) // Fixed dependency by adding fetchData which depends on core filters

  const filteredCoins = useMemo(() => {
    let result = [...coins]

    // 1. Network filtering (secondary check in case API search was broad)
        ...
        return (
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatCard title="Verified Tokens" value={`${stats.safePairs}%`} icon={ShieldCheck} color="text-green-500" />
              <StatCard title="Market Volatility" value="High" icon={Activity} color="text-red-500" />
            </div>

            <div className="bg-card rounded-xl border border-border overflow-hidden">
              <div className="p-4 border-b border-border flex justify-between items-center">
                <h2 className="text-lg font-bold">Global Trending</h2>
            ...
            {address ? (
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="bg-secondary text-foreground px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 border border-border"
                >
